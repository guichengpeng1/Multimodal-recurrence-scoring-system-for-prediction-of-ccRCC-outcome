...
train_labels = []
for tile in X_trains:
    label = classes_to_index[Path(tile).parent.name]
...
def parse_fn_MultiInput(example):
    example_fmt= {
        #"shape":tf.io.FixedLenFeature([3],tf.int64,default_value=[0,0,0]),
         ...
         #"patch_name":tf.io.FixedLenFeature([],tf.string,default_value="")
    }
...
def plot_metrics(history,index=None):
    ...
    for n, metric in enumerate(metrics):
        ...
        plt.ylabel(name)
        if metric == 'loss':
            plt.ylim([min(plt.ylim()), plt.ylim()[1]])
        elif metric == 'auc'or "prc":
            plt.ylim([0.6,1])
        else:
...
def Base_Model(MODEL=None,weights=None):
    ...
    if MODEL =="inception_resnet":
        ...
        base_model = tf.keras.applications.MobileNetV2(input_shape=IMG_SHAPE,
                                    include_top=False,
            weights=weights)
    elif MODEL =="dense_net_121":
        base_model = tf.keras.applications.DenseNet121(input_shape=IMG_SHAPE,
                                    include_top=False,
            weights=weights)
    elif MODEL =="resnet_101_v2":
        base_model = tf.keras.applications.ResNet101V2(input_shape=IMG_SHAPE,
                                    include_top=False,
            weights=weights)


    elif MODEL =="resnet_50_v2":
        base_model = tf.keras.applications.ResNet50V2(input_shape=IMG_SHAPE,
                                    include_top=False,
            weights=weights)    


    elif MODEL =="resnet_152_v2":
        ...
        base_model = tf.keras.applications.DenseNet201(input_shape=IMG_SHAPE,
                                    include_top=False,
            weights=weights)
    elif MODEL == "efficient_net_b7":
        base_model = tf.keras.applications.EfficientNetB7(input_shape=IMG_SHAPE,
                                    include_top=False,
            weights=weights)
    elif MODEL == "efficient_net_b6":
        base_model = tf.keras.applications.EfficientNetB6(input_shape=IMG_SHAPE,
                                    include_top=False,
            weights=weights)
    elif MODEL == "efficient_net_b5":
        ...
        base_model = tf.keras.applications.MobileNetV3Large(input_shape=IMG_SHAPE,
                                    include_top=False,
            weights=weights)
    elif MODEL == "mobile_net_v3_small":
...
class NoisyAnd(Layer):
    ...
    def build(self, input_shape):
        ...
        self.a = tf.constant(10,dtype=tf.float32)  # fixed, controls the slope of the activation
...
PARA["base_model_trainable"]= True
PARA["fc_layers"] = 'dropout(0.5)'
PARA["weight_decay"]='0.15'
PARA['optimizer']='AdamW'
PARA["initial_lr"]='0.001'
PARA["class_weight"] = False


def get_files(subset):
    files = glob.glob(f"{tfrec_path}/*.tfrecords")
    files = [i  for j in subset for i in files if Path(j).name in i]
    return files
...
train_ds = train_dataset.interleave(map_func=tf.data.TFRecordDataset,cycle_length=CYCLE_LEN,block_length=BLOCK_LEN,num_parallel_calls=AUTOTUNE).map(parse_fn_MultiInput,num_parallel_calls=AUTOTUNE).map(augment_MultiInput,num_parallel_calls=AUTOTUNE).repeat().batch(BATCH_SIZE,drop_remainder=True).prefetch(AUTOTUNE)


"""load all tiles of validation cases"""

X_valids = [str(i) for i in list(X_valids)]
all_files = glob.glob(f"*/{level}/*.tfrecords") # glob all tfrecords
...
"""Get validation cases'labels"""
valid_labels = []
for tiles in valid_tiles:
    label = classes_to_index[Path(tiles[0]).parent.parent.parent.name]
...
"""Model to save single layers' weights"""
    ...
    with tf.GradientTape() as tape:
        for num in range(images.shape[1]):
            ...
            if num ==0:
                ...
                outs_all = tf.concat([outs_all,out],1)
        pool_output = pool(outs_all)
        
        #tf.print("pool_output:",pool_output,output_stream=sys.stdout)
       
        dense_output = dense(pool_output)
        loss_1 = loss_fn(labels,dense_output)
        loss_2 = TwoBinaryCE(labels,pool_output)
        loss = loss_1 +loss_2
